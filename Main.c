...
#define ARM_POTENTIOMETER_STATIONARY_GOAL_VALUE		1800
...
#define ARM_POTENTIOMETER_OFFSET					-700
...
#define MOGO_LIFT_POTENTIOMETER_EXTENDED_VALUE	660
#define MOGO_LIFT_POTENTIOMETER_RETRACTED_VALUE	2860
#define MOGO_LIFT_POTENTIOMETER_HALFWAY_VALUE	2060
...
#define MINI_4_BAR_POTENTIOMETER_EXTENDED_MOGO_VALUE	1700
#define MINI_4_BAR_POTENTIOMETER_STATIONARY_VALUE	1567
...
#define GOLIATH_REST_POWER		30
#define GOLIATH_RETRACTED_POWER	30
...
#define MENU_LIST_AUTON_LENGTH 			15
...
enum StateExtension { STATE_EXTENSION_EXTENDED, STATE_EXTENSION_RETRACTED, STATE_EXTENSION_HALFWAY, STATE_EXTENSION_STATIONARY, STATE_EXTENSION_MOGO_EXTENDED };
...
MenuItem menuItemAutonGoBack, menuItemAuton22P, menuItemAuton24P, menuItemAuton26P, menuItemAutonStack, menuItemAuton12P, menuItemAuton14P, menuItemAuton7P, menuItemAuton9P, menuItemAutonCRam, menuItemAutonC20P, menuItemAutonC5P, menuItemAutonR10P, menuItemAutonNone, menuItemProgSkills1;
//side menu items
...
void populateMenuItems() {
	...
	menuItemAuton26P.name = "Auton26P";
	menuItemAuton26P.id = ++id;
	menuItemAuton26P.idx = ++i;
	menuItemAuton26P.isDirectional = true;
	menuListAuton[i] = &menuItemAuton26P;

	menuItemAutonStack.name = "AutonStack";
	menuItemAutonStack.id = ++id;
	menuItemAutonStack.idx = ++i;
	menuItemAutonStack.isDirectional = true;
	menuListAuton[i] = &menuItemAutonStack;
	...
	menuItemAutonCRam.name = "AutonCRam";
	menuItemAutonCRam.id = ++id;
	menuItemAutonCRam.idx = ++i;
	menuItemAutonCRam.isDirectional = true;
	menuListAuton[i] = &menuItemAutonCRam;

	menuItemAutonC20P.name = "AutonC20P";
	menuItemAutonC20P.id = ++id;
	menuItemAutonC20P.idx = ++i;
	menuItemAutonC20P.isDirectional = true;
	menuListAuton[i] = &menuItemAutonC20P;

	menuItemAutonC5P.name = "AutonC5P";
	menuItemAutonC5P.id = ++id;
	menuItemAutonC5P.idx = ++i;
	menuItemAutonC5P.isDirectional = true;
	menuListAuton[i] = &menuItemAutonC5P;
...
task tMini4Bar()
{
	...
	if (tMini4BarGoalState == STATE_EXTENSION_EXTENDED) mini4BarExtend(waitForTMini4BarPID, tMini4BarPIDOnStall);
	else if (tMini4BarGoalState == STATE_EXTENSION_RETRACTED) mini4BarRetract(waitForTMini4BarPID, tMini4BarPIDOnStall);
	else if (tMini4BarGoalState == STATE_EXTENSION_STATIONARY) mini4BarPIDControl(MINI_4_BAR_POTENTIOMETER_STATIONARY_VALUE, waitForTMini4BarPID, tMini4BarPIDOnStall, tMini4BarIsDataLogged);
	else if (tMini4BarGoalState == STATE_EXTENSION_MOGO_EXTENDED) mini4BarPIDControl(MINI_4_BAR_POTENTIOMETER_EXTENDED_MOGO_VALUE, waitForTMini4BarPID, tMini4BarPIDOnStall, tMini4BarIsDataLogged);
...
task autonomous()
{
	...
	if ( (*selectedProgram).id == menuItemAuton22P.id || (*selectedProgram).id == menuItemAuton7P.id || (*selectedProgram).id == menuItemAuton9P.id || (*selectedProgram).id == menuItemAuton12P.id || (*selectedProgram).id == menuItemAuton24P.id || (*selectedProgram).id == menuItemAuton14P.id || (*selectedProgram).id == menuItemAuton26P.id)
	{
		setGoliathMotorPower(GOLIATH_INTAKE_POWER);
		startTMini4Bar(STATE_EXTENSION_MOGO_EXTENDED, WAIT, ON_STALL_EXIT);
		while (getMini4BarSensorValue() < MINI_4_BAR_POTENTIOMETER_RETRACTED_VALUE + 300) { wait1Msec(1); }

		startTArmPID(ARM_POTENTIOMETER_MIN_VALUE + 400, WAIT, ON_STALL_EXIT);
		startTMoGoLift(STATE_EXTENSION_EXTENDED, ON_STALL_EXIT);
		while (getMoGoLiftSensorValue() < correctMoGoLiftGoalPoint(MOGO_LIFT_POTENTIOMETER_EXTENDED_VALUE + 900) ) { wait1Msec(1); }
		startTDrivePID(1430, MODE_ACCURATE, ON_STALL_EXIT);
		waitForTDrive();

		stopTask(tArmPIDControl);
		setArmMotorPower(-127);
		while (getArmSensorValue() > correctArmGoalPoint(ARM_POTENTIOMETER_MIN_VALUE + 150)) { wait1Msec(1); }
		setArmMotorPower(0);
		...
		if ((*selectedProgram).id == menuItemAuton26P.id)
		{
			...
			startTArmPID(lastStackedConeSensorValue + 150, WAIT, ON_STALL_EXIT);
			wait1Msec(200);
			setGoliathMotorPower(GOLIATH_OUTTAKE_POWER);
			wait1Msec(300);
			waitForTGyroPID();
		...
		if ( (*selectedProgram).id == menuItemAuton22P.id || (*selectedProgram).id == menuItemAuton24P.id || (*selectedProgram).id == menuItemAuton26P.id)
		{
			startTArmPID(lastStackedConeSensorValue + 50, WAIT, ON_STALL_EXIT);
			if (autonomousSide == SIDE_LEFT) startTGyroPID(46, MODE_MOGO, ON_STALL_EXIT);
			else if (autonomousSide == SIDE_RIGHT) startTGyroPID(-46, MODE_MOGO, ON_STALL_EXIT);
			wait1Msec(300);
			setGoliathMotorPower(-70);
			wait1Msec(200);

			if ( (*selectedProgram).id == menuItemAuton24P.id && correctArmGoalPoint(lastStackedConeSensorValue) > correctArmGoalPoint(ARM_POTENTIOMETER_MIN_VALUE + 50 ) )
			{
				startTArmPID(lastStackedConeSensorValue + 400, WAIT, ON_STALL_EXIT);
			}
			else
			{
				startTArmPID(1200, WAIT, ON_STALL_EXIT);
			}

			waitForTGyroPID();

			startTDrivePID(-600, MODE_ACCURATE, ON_STALL_EXIT);
			waitForTDrive();

			if (autonomousSide == SIDE_LEFT) startTGyroPID(150, MODE_MOGO, ON_STALL_EXIT);
			else if (autonomousSide == SIDE_RIGHT) startTGyroPID(-150, MODE_MOGO, ON_STALL_EXIT);
			waitForTGyroPID();
	...
	else if ( (*selectedProgram).id == menuItemAutonCRam.id || (*selectedProgram).id == menuItemAutonC20P.id || (*selectedProgram).id == menuItemAutonC5P.id )
	{
		setGoliathMotorPower(GOLIATH_REST_POWER);
		...
		stopTask(tArmPIDControl);
		setArmMotorPower(-127);
		while (getArmSensorValue() > correctArmGoalPoint(1610) ) { wait1Msec(1); }
		setArmMotorPower(0);
		setGoliathMotorPower(-60);
		wait1Msec(200);
		setGoliathMotorPower(0);
		...
		stopTask(tArmPIDControl);
		setArmMotorPower(-127);
		while (getArmSensorValue() > correctArmGoalPoint(ARM_POTENTIOMETER_MIN_VALUE) ) { wait1Msec(1); }
		setArmMotorPower(0);
		waitForTDrive();
		...
		{
			startTDrivePID(-293, MODE_ACCURATE, ON_STALL_EXIT);
			waitForTDrive();

			startTMoGoLift(STATE_EXTENSION_EXTENDED, ON_STALL_EXIT);
			if (autonomousSide == SIDE_LEFT) startTGyroPID(94, MODE_ACCURATE, ON_STALL_EXIT);
			else if (autonomousSide == SIDE_RIGHT) startTGyroPID(-94, MODE_ACCURATE, ON_STALL_EXIT);
			waitForTGyroPID();
			...
			else if ((*selectedProgram).id == menuItemAutonC5P.id)
			{
				...
				while (getMoGoLiftSensorValue() > correctMoGoLiftGoalPoint(MOGO_LIFT_POTENTIOMETER_EXTENDED_VALUE + 400) ) { wait1Msec(1); }
				startTDrivePID(150, MODE_ACCURATE, ON_STALL_EXIT);
				waitForTDrive();
				waitForTMoGoLift();
				...
				startTDrivePID(-1140, MODE_ACCURATE, ON_STALL_EXIT);

				while (getArmSensorValue() < correctArmGoalPoint(lastStackedConeSensorValue + 150)) { wait1Msec(1); }
				startTMini4Bar(STATE_EXTENSION_RETRACTED, WAIT, ON_STALL_EXIT);
				waitForTMini4Bar();
				stopTask(tArmPIDControl);
				actionUntilUnderGoalPoint(A_ARM, ARM_POTENTIOMETER_MIN_VALUE, -127);
				...
				if (autonomousSide == SIDE_LEFT) startTGyroPID(280, MODE_MOGO, ON_STALL_EXIT);
				else if (autonomousSide == SIDE_RIGHT) startTGyroPID(-280, MODE_MOGO, ON_STALL_EXIT);
				startTArmPID(lastStackedConeSensorValue + 300, WAIT, ON_STALL_EXIT);

				while (abs(getGyroSensorValue()) < 200) { wait1Msec(1); }
				startTMoGoLift(STATE_EXTENSION_HALFWAY, ON_STALL_EXIT);
				waitForTGyroPID();
...
task Goliath()
{
	while (true)
	{
		if (!lockControls && !isJoystickLCDMode() && !isMoGoStackConeMacroActive && !isArmReadyMacroActive)
		{
			...
			{
				if ( getArmSensorValue() < ARM_POTENTIOMETER_MIN_VALUE ) setGoliathMotorPower(127);
