...
#pragma config(Motor,  port6,           motorArmRightTop, tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port7,           motorArmLeftRightBottom,  tmotorServoContinuousRotation, openLoop) // Expander A and B
...
#pragma config(Motor,  port9,           motorArmLeftTop, tmotorServoContinuousRotation, openLoop, reversed)
...
{
	motor[motorArmLeftTop] = motorPower;
	motor[motorArmRightTop] = motorPower;
	motor[motorArmLeftRightBottom] = motorPower;
}
...
task Goliath()
{
	while (true)
	{
		if (!lockControls && !isJoystickLCDMode() && !isMoGoStackConeMacroActive && !isArmReadyMacroActive)
		{
			...
			{
				if (motor[motorArmLeftTop] <= 0) setGoliathMotorPower(50);
				else if (motor[motorArmLeftTop] > 0) setGoliathMotorPower(15);
...
short getCurrentFlag()
{
	...
	if (abs(motor[motorArmLeftTop]) > 0) flag += FLAG_BIT_ARM_ACTIVE;
